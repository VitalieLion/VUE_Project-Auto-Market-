...
const store = createStore({
  state: {
    ...
    cars: [
      ...
      },
      {
        ...
        car_year: 2010,
        url:
          "https://images.hgmsites.net/hug/2018-bmw-6-series_100609718_h.jpg",
      },
      {
        id: 31,
        car_make: "Pontiac",
    ...
    favoriteCar: JSON.parse(localStorage.getItem("favoriteCar")) || [],
  },
  ...
  mutations: {
    ...
    updateCar(state, data) {
      state.cars[state.cars.findIndex((car) => car.id === data.id)] = data;
    },
    addNewCar(state, data) {
      state.cars = data;
    },
    addFavoriteCar(state, car) {
      state.favoriteCar = [
        ...state.favoriteCar,
        state.cars.find((item) => item.id === car),
      ];

      localStorage.setItem("favoriteCar", JSON.stringify(state.favoriteCar));
    },
    getPrice(state, car) {
      state.cars = car
    }
  ...
  actions: {
    ...
    addNewCar({ commit, state }, Car) {
      Car.id = state.cars.length + 1;
      commit("addNewCar", [Car, ...this.state.cars]);
    },
    deleteCar(state, car) {
      let carIndex = state.favoriteCar.findIndex((item) => item.id === car);
      state.favoriteCar.splice(carIndex, 1);
    },
    getPrice({ commit, state }) {
      commit('getPrice', state.cars.map((item) => {
        return{...item, randomPrice:`${ Math.round(Math.random()*100)}000$`}
      }))
    }
  },
});
...
//   state.users.splice(userIndex, 1,{...state.users[userIndex],first_name:data.name.split(' ')[0],
//   last_name:data.name.split(' ')[1]})
//   console.log(state.users)

// },
// deleteUser(state, itemS) {
//   state.users = state.users.filter(user => user.id !== +itemS);
// }
